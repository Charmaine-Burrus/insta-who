...
function doUpload() {
  ...
  file1 = document.getElementById("upload");
  image1 = new SimpleImage(file1);
...
function filterTrans() {
  for (var pixel of image1.values()) {
    var alpha = pixel.getAlpha();
    pixel.setAlpha(alpha/2);
  }
}
...
function makeFaded() {
  for (var pixel of image1.values()) {
    ...
    if (x > w/3 && x < 2 * (w/3)) {
      pixel.setAlpha(alpha*(2/3))
    }
    if (x > 2 * (w/3)) {
      pixel.setAlpha(alpha/3);
    }
  }
 image1.drawTo(canvas1);
}
...
function addBorder() {
    var thicknessinput = document.getElementById("sldr");
    var t = thicknessinput.value;
    var w = image1.getWidth();
    var h = image1.getHeight();
    clearCanvas();
    image1.drawTo(canvas1);
    ctx1.fillStyle = "black";
    ctx1.fillRect(0,0,t,h);
    ctx1.fillRect(0,0,w,t);
    ctx1.fillRect(w,h,-t,-h);
    ctx1.fillRect(w,h,-w,-t);
}

function doBlur() {
    var img2 = new SimpleImage(image1.getWidth(),image1.getHeight());
    for (var pixel of img2.values()) {
        var R = Math.random();
        var x = pixel.getX();
        var y = pixel.getY();
        var bipixel = image1.getPixel(x,y);
        if (R < 0.5) {
            img2.setPixel(x,y,bipixel);
        }
        if (R >= 0.5) {
            var newx = (x + Math.floor(Math.random()*10));
            var newy = (y + Math.floor(Math.random()*10));
            if (newx < image1.getWidth() && newy < image1.getHeight()) {
                var difbipixel = image1.getPixel(newx,newy);
                img2.setPixel(x,y,difbipixel);
            }
            else {
                img2.setPixel(x,y,bipixel);
            }
